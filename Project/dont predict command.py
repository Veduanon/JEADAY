...
def record_and_recognize_audio(*args: tuple):
    ...
    with microphone:
        ...
        try:
            ...
            play_voice_assistant_speech(translator.get("Проверьте подключение микрофона"))
            traceback.print_exc()
            return

        # использование online-распознавания через Google (высокое качество распознавания)
        try:
            print("Распознование сказанного...")
            recognized_data = recognizer.recognize_google(audio, language="ru").lower()

        except speech_recognition.UnknownValueError:
            print("Что вы сказали?")
...
def play_greetings(*args: tuple):
    ...
    greetings = [
        ...
        translator.get("Hi there {}! How can i help you?").format(person.name)
    ]
...
open_weather_map=OWM("c5a5170700e865e2c1b18316f9dbefa4")
...
def get_weather_forecast(*args: tuple):
    ...
    # запрос данных о текущем состоянии погоды
        ...
        observation = weather_manager.weather_at_place("Astana")
    ...
    # вывод логов
    print(colored("Weather in " + "Astana" +
                  ":\n * Status: " + status +
    ...
    play_voice_assistant_speech(translator.get("It is {0} in {1}").format(status, "Nur-Sultan"))
...
commands = {
    ("салам", "здарова", "привет"): play_greetings,
    ("bye", "пока"): play_farewell_and_quit,
    ("найди", "найди в гугле"): search_for_term_on_google,
    ("видео", "запусти видео", "включи видео"): search_for_video_on_youtube,
    ("что означает", "определение", "википедия"): search_for_definition_on_wikipedia,
    ("перевод", "перевести", "переведи"): get_translation,
    ("какая", "погода сейчас", "погода", "прогноз"): get_weather_forecast,
    ("пробей", "контакт"): run_person_through_social_nets_databases,
