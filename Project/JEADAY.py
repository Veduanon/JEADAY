...
def get_weather_forecast(*args: tuple):
    ...
    # вывод логов
    print(colored("Погода в " + "Nur-Sultan" +
                  ":\n * Небо: " + status +
                  "\n * Скорость ветра (м/сек): " + str(wind_speed) +
                  "\n * Температура (Целсьий): " + str(temperature), "yellow"))
...
config = {
    "intents": {
        "greeting": {
            "examples": ["привет", "салам", "доброе утро", "добрый день"],
            "responses": play_greetings
        },
        "farewell": {
            "examples": ["пока", "досвидания", "увидимся", "довстречи"],
            "responses": play_farewell_and_quit
        },
        "google_search": {
            "examples": ["найди в гугле", "google", "загугли"],
        ...
        "wikipedia_search": {
            "examples": ["найди определение", "найди на википедии", "определение слова"],
...
def get_intent(request):
    ...
    if best_intent_probability > 0.159:
        return best_intent
